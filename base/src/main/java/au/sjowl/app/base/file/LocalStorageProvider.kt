...
class LocalStorageProvider : DocumentsProvider() {
    ...
    override fun queryRoots(projection: Array<String?>?): Cursor {
        ...
        row.add(Root.COLUMN_ICON, R.drawable.ic_mtrl_chip_close_circle)
    ...
    override fun openDocumentThumbnail(
        ...
        val bitmap: Bitmap = BitmapFactory.decodeFile(documentId, options)
        ...
        try {
            ...
            loge("Error writing thumbnail", e)
            return null
        } finally {
            if (out != null) try {
                out.close()
            } catch (e: IOException) {
                loge("Error closing thumbnail", e)
            }
        }
    ...
    private fun includeFile(result: MatrixCursor, file: File) {
        ...
        var flags =
            if (file.canWrite()) Document.FLAG_SUPPORTS_DELETE or Document.FLAG_SUPPORTS_WRITE else 0
